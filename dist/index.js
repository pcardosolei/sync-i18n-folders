...
function sync({ check: isReportMode = false, folder, primary: primaryLanguage = "en", createResources: createFiles = [], space: jsonSpacing = 4, lineEndings = "LF", finalNewline = false, newKeysEmpty = false, }) {
    const directories = (0, common_1.getDirectories)(folder);
    ...
    if (!primaryLanguageExists) {
        ...
        console.log(`Found ${primaryLanguage} folder`);
    }
    const primaryLanguageFiles = (0, common_1.getJSONDirectoryFiles)(`${folder}/${primaryLanguage}`);
    for (const file of primaryLanguageFiles) {
        const content = (0, common_1.readFileToJSON)(`${folder}/${primaryLanguage}/${file}`);
        for (let keys of Object.keys(content)) {
            console.log((0, utils_1.isObject)(content[keys]));
            console.warn(keys, content[keys]);
        }
    }
    let targetLanguage;
    ...
    // //   const sourceValue: LocalizationValue =
    // //     source[key as typeof LocalizationVaue];
    ...
    //   if (isObject(sourceValue)) {
    //     target[key] = {};
    //     syncObjects(sourceValue, target[key as keyof Object]);
    //   } else if (
