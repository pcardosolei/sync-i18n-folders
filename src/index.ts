...
export default function sync({
  ...
  for (const file of primaryLanguageFiles) {
    const content = readFileToJSON(`${folder}/${primaryLanguage}/${file}`);
    for (let keys of Object.keys(content)) {
      console.log(isObject(content[keys]));
    ...
    if (target.hasOwnProperty(key)) {
      if (areSameTypes(sourceValue, targetValue)) {
        if (isObject(sourceValue)) {
          ...
          // ignoring the plurals for now need to understand that.
  ...
  function copyValue(source: JSONObject, target: JSONObject, key: string) {
